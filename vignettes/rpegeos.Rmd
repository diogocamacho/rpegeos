...
```{r calculate_tfidf}
tfidf_dtm <- tfidf(data_matrix = dtm)
```
...
```{r run_enrichment}
qgs <- G$ENTREZID[which(res$logFC > 1 & res$adj.P.Val < 0.01)] # <--- query gene set

rpegeos_enrichment <- enrich_geneset(gene_set = qgs,
...
plot_enrichment(results_df = rpegeos_enrichment, random_thr = 0.001, max_pathways = 15)
